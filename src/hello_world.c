...
void out_failed_handler(DictionaryIterator *failed, AppMessageResult reason, void *context) {
  ...
  text_layer_set_text(text_layer, "Recv Fail");
}
...
void in_received_handler(DictionaryIterator *received, void *context) {
  ...
  text_layer_set_text(text_layer, "Phone Clicked");
}
...
//  double jerk = sqrt((accel.x - prevX)^2 + (accel.z - prevZ)^2 + (accel.z - prevZ)^2); 
 
  double jerk = accel.z - prevZ;
  ...
  else
  {
    ...
    text_layer_set_text(text_layer, "Stable.");
